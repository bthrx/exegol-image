...
function gf_install() {
  ...
  git -C /opt/tools clone https://github.com/tomnomnom/gf
  echo 'source /opt/tools/gf/gf-completion.zsh' | tee -a ~/.zshrc
  cp -rv /opt/tools/gf/examples/* ~/.gf #copy from the examples folder in /opt/tools/gf
  ...
  cp -rv /opt/tools/Gf-Patterns/* ~/.gf
}
...
function install_postman() {
  wget https://dl.pstmn.io/download/latest/linux64 -O /opt/tools/postman-linux-x64.tar.gz
  tar -xvzf /opt/tools/postman-linux-x64.tar.gz -C /opt/tools
  rm /opt/tools/postman-linux-x64.tar.gz
  ln -s /opt/tools/Postman/app/Postman /usr/bin/Postman
}

function install_android_studio() {
  fapt openjdk-11-jre
  wget https://redirector.gvt1.com/edgedl/android/studio/ide-zips/2021.2.1.16/android-studio-2021.2.1.16-linux.tar.gz -O /opt/tools/android-studio.tar.gz
  tar -xvzf /opt/tools/android-studio.tar.gz -C /opt/tools
  rm /opt/tools/android-studio.tar.gz
  ln -s /opt/tools/android-studio/bin/studio.sh /usr/bin/android-studio
}
...
function install_mobsf() {
  ...
  fapt python3-venv
  fapt python3-pip 
  fapt build-essential
  fapt libffi-dev
  fapt libssl-dev
  fapt libxml2-dev
  fapt libxslt1-dev
  fapt libjpeg62-turbo-dev
  fapt zlib1g-dev
  fapt wkhtmltopdf
...
function install_wappalyzer-cli() {
  git -C /opt/tools clone https://github.com/gokulapap/wappalyzer-cli
  pip3 install /opt/tools/wappalyzer-cli
}

function install_get_schemas() {
  git -C /opt/tools clone https://github.com/teknogeek/get_schemas
  pip3 install -r /opt/tools/get_schemas/requirements.txt
  ln -s /opt/tools/get_schemas/get_schemas.py /usr/bin/get_schemas
}

function install_truffleHog() {
  git -C /opt/tools clone https://github.com/trufflesecurity/trufflehog.git
  cd /opt/tools/trufflehog
  go install 
}

function install_gitleaks() {
  git -C /opt/tools clone https://github.com/zricethezav/gitleaks.git
  cd /opt/tools/gitleaks
  go install
}

function install_radamsa() {
  git -C /opt/tools clone https://gitlab.com/akihe/radamsa.git
  cd /opt/tools/radamsa
  make
  ln -s /opt/tools/radamsa/bin/radamsa /usr/bin/radamsa
}

function install_deeplink-fuzz() {
  git -C /opt/tools clone https://github.com/B3nac/deeplink-fuzz.sh.git
  ln -s /opt/tools/deeplink-fuzz.sh/deeplink-fuzz.sh /usr/bin/deeplink-fuzz
}

function install_pastebin-scraper() {
  git -C /opt/tools clone https://github.com/streaak/pastebin-scraper.git
  ln -s /opt/tools/pastebin-scraper/scrape.sh /usr/bin/pastebin-scraper  
}

function install_cloudlist() {
  go install -v github.com/projectdiscovery/cloudlist/cmd/cloudlist@latest
}

function install_paramspider() {
  git -C /opt/tools clone https://github.com/devanshbatham/ParamSpider
  pip3 install -r /opt/tools/ParamSpider/requirements.txt
  ln -s /opt/tools/ParamSpider/paramspider.py /usr/bin/paramspider
}

function install_crawljax() {
  mkdir /opt/tools/crawljax
  wget https://github.com/crawljax/crawljax/releases/download/crawljax-4.1/crawljax-cli-4.1.zip -O /opt/tools/crawljax.zip
  unzip crawljax.zip -d /opt/tools/crawljax
  echo 'alias crawljax="java -jar /opt/tools/crawljax/crawljax-cli-4.1/crawljax-cli-4.1.jar"' >> ~/.zshrc
  source ~/.zshrc
}

function install_firebase_enum() {
  git -C /opt/tools clone https://github.com/Sambal0x/firebaseEnum.git
  pip3 install -r /opt/tools/firebaseEnum/requirements.txt
  ln -s /opt/tools/firebaseEnum/firebaseEnum.py /usr/bin/firebaseEnum
}

function install_ipfuscator() {
  git -C /opt/tools clone https://github.com/vysec/ipfuscator
  echo 'alias ipfuscator="python /opt/tools/ipfuscator/ipfuscator.py"' >> ~/.zshrc
  source ~/.zshrc
}
...
#Function to install the rest of my tools
function install_bthrx_tools() {
  fapt nmap
  install_waybackurls
  install_jadx
  gf_install
  ...
  install_mobsf
  install_wappalyzer-cli
  install_get_schemas
  install_truffleHog
  install_gitleaks
  install_radamsa
  install_deeplink-fuzz
  install_pastebin-scraper
  install_cloudlist 
  install_paramspider
  install_crawljax
